...
contract NTCitizenDeploy is ERC1155, IERC721Receiver, ReentrancyGuard, Ownable {
    ...
    mapping(uint256 => uint256) public tokenStakeCompletionDate;
    mapping(address => uint256) public countOfTokensStakedByOwner;
    mapping(uint256 => uint256) public selectedStakingRewardsByToken;
    ...
    function airdrop(address[] calldata recipients, uint256 count) external onlyAdmin nonReentrant 
    {
        for(uint256 i = 0; i < recipients.length; i++)
        {
            _mint(recipients[i], currentMintableToken, count, "");
        }
    }

    function toggleStakingOpen() external onlyAdmin 
    {
        stakingOpen = !stakingOpen;
    }

    function setAdmin(address adminToToggle) external onlyOwner 
    {
        admins[adminToToggle] = !admins[adminToToggle];
    }

    function stakeTitan(uint256 titanId, uint256 stakingPeriod) public nonReentrant 
    {
        IERC721 _sstContract = IERC721(SSTContract);
        _sstContract.transferFrom(_msgSender(), address(this), titanId);
        tokenStakeCompletionDate[titanId] = block.timestamp + stakingLengths[stakingPeriod];
        selectedStakingRewardsByToken[titanId] = stakingPeriod;
        stakedTokenOwner[titanId] = _msgSender();
    ...
    {
        IERC721 titanEnumerable = IERC721(SSTContract);
        return(titanEnumerable.ownerOf(titanId) == _msgSender());
    }

    function setTitanContract(address contractAddress) public onlyOwner 
    {
        SSTContract = contractAddress;
    }
    ...
    function setCurrentToken(uint256 currentId) external onlyOwner 
    {
        currentMintableToken = currentId;
    }

    function toggleApprovedBurningContract(address burningContract) external onlyOwner
    {
